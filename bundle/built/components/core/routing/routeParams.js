...
var RouteParams = (function () {
    ...
    _createClass(RouteParams, [{
        ...
        value: function build() {
            var args = [];
            if (Reflect.hasMetadata(_paramTypes_1.allianceParams, this.target, this.key)) {
                this.paramNames = paramNames_1.getParamNames(this.func);
                var argsList = Reflect.getMetadata(_paramTypes_1.allianceParams, this.target, this.key).sort(function (a, b) {
                ...
                try {
                    for (var _iterator = argsList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        ...
                        switch (arg.type) {
                            case _paramTypes_1.allianceParamsType.PathVariable:
                                this.pathVariable(arg);
                                break;
                            case _paramTypes_1.allianceParamsType.RequestParam:
                                this.requestParam(arg);
                                break;
                            case _paramTypes_1.allianceParamsType.BodyParam:
                                this.bodyParam(arg);
                                break;
                            case _paramTypes_1.allianceParamsType.Inject:
                                this.inject(arg);
                                break;
                        }
